...
- **未验证**：真实**下单**从未在真机跑过（下单闸默认关闭）。
- **未达标**：业务跑道（见第 1 节）+ 若干代码债（见第 7 节）——**这些没清之前不要放真钱**。

一句话：**技术链路通了，但「可以放真钱」还差跑道验证和几项安全硬化。**
...
- [ ] 已在券商开户（本项目实测券商：**江海证券**）
- [ ] 已申请并开通「**QMT / 极速交易（量化）接口**」权限
- [ ] 满足资产门槛（多数券商 ≥50 万）
- [ ] 拿到并安装了 QMT 客户端（含 `XtMiniQmt.exe` 交易端 + `miniquote.exe` 行情）

### 1.3 代码债（见第 7 节，逐项清完再上真钱）
- [ ] 第 7 节列出的 HIGH/MEDIUM 项已修复或明确接受
...
- **真实下单/撤单/成交回报**从未在真机跑过（一直是只读 + mock）。阶段 D 的手动冒烟就是补这一步。
