...
- **后续（紧接）：声明式"策略规则"生成**——把 ② DSL 扩到信号层（`Cross/阈值/比较` 等布尔/信号算子），让 LLM 生成**可解析(不执行)的声明式策略**，复用 ⑤ walk-forward 闸门验证、跑赢基线才采纳。与因子同一套安全模型（解析而非 `exec`，绝不上任意代码生成/沙箱）。这是 ⑥ 跑通后最自然的延伸。
- 其它后续：多轮 LLM 反思迭代、因子去相关组合优化、IC 衰减监控自动下架、字符串 parser 反哺 ② 让所有因子都可配置化。
- **明确不做**：生成任意 Python 策略代码（需 `exec` + 沙箱，远程代码执行风险）——除非有强理由且投入真沙箱。
