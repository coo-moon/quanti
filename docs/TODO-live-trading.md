...
> **⚠️ 本文是接入前的早期规划稿(2026-05-19)。截至 2026-07-08,QMT 直连 xtquant 桥(`bridge/xt_direct_backend.py`)已建成、对江海证券 QMT 实盘完成【只读】端到端验证,接实盘代码债已全部清零(PR #125–#134)。实盘现状、操作步骤、风险边界以 [`live-trading-runbook.md`](live-trading-runbook.md) 为准;本文 §5「代码侧」多数条目已完成(实现机制常与所列不同),仅留作历史参考。**

**当前状态**：日常仍走 PaperBroker 模拟盘;实盘链路已就绪(只读已验证,真实下单默认关、冒烟待盘中)。
**决定时间**：2026-05-19。
**计划复盘日**：~~2026-08-19~~ —— 账本近期被 reset,业务跑道「连续 ≥60 交易日」从**最后一次重置**起算,最早约 **2026-09~10** 才达标(原 08-19 已不适用)。
**前置条件**：PaperBroker **连续、不被重置**运行满 ≥60 交易日,累计年化 ≥ 目标 70% 且最大回撤未破 -20%、风控链路真触发过,才考虑接实盘;否则继续模拟。
