...
| **告警推送** | 关键事件(组合熔断/实盘未知单/bridge 掉线/体检异常/LLM 不可用/急停)主动推 webhook——飞书/钉钉/企业微信/Server酱/通用 JSON 按 URL 自适配；Dashboard「告警通道」面板或 env `QUANTI_ALERT_WEBHOOK` 配置,同类事件 10 分钟去抖,推送失败绝不影响交易链路 |
...
| **LLM 因子挖掘** | LLM 提因子表达式 → 安全解析 → 训练/OOS rank-IC 闸门去冗余 → 采纳入自演化因子库，可选注入实盘排序；每日重评沉淀 IC 快照，漂移监视器自动标记衰减/退役因子(`quanti factor-watch`) |
...
设计哲学：**规则优先，LLM 只做加法**。回测、因子打分、风控全部保持确定性可审计；LLM 的职责限定为对系统产出的候选做"判断"——倾斜排序、收缩仓位、补充上下文。硬风控（单票 20% / 行业 30% / 止损线 / 组合 -30% 熔断，均可在 UI 调整）LLM 永远无法绕过；全部开关默认关闭，关闭时行为与纯规则路径完全一致。
...
**供应商**：`llm_provider` 支持 `deepseek`（默认模型 `deepseek-flash`，OpenAI 兼容接口，零额外依赖，`export DEEPSEEK_API_KEY=...` 即可）和 `anthropic`（`pip install -e ".[llm]"` + `ANTHROPIC_API_KEY`）。`claude-*` 模型名在 DeepSeek 路径下自动重映射；思考模式与强制工具调用的兼容性已在客户端处理（结构化输出自动关思考，辩论等自由文本保留思考并按默认最高档 `reasoning_effort: max` 下发）。
...
在确定性**硬限**（单票 20% / 行业 30% / 单票 ATR 止损 + -15% 地板 / 组合 -30% 熔断）之上，再叠加一层从历史事实推导的**软锁**——只临时禁止**新建仓**、从不强制卖出，回测与实盘共用同一套纯逻辑：
...
- [x] **LLM 多智能体增强层**：新闻情绪 / 多空辩论 / 风控三角 / 反思记忆（DeepSeek `deepseek-flash` 默认，Anthropic 可选）
...
- [x] **可组合风控 protections**：StoplossGuard + MaxDrawdown 软锁（在 -30% 硬熔断之上）
- [x] **回测≡实盘一致性加固（2026-06-22 审计修复）**：回测应用组合回撤熔断（C1）、建仓复用实盘 sizing 读 signal.strength（C2）、回测滑点对齐实盘 FlatSlippage 10bps（C4）、单 bar 成交额参与率上限封顶巨单瞬时成交（B1）、PaperBroker T+1 按建仓批次冻结当日买入（F1）
