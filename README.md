...
| **可组合风控** | 硬限（单票/行业/总仓/止损）之上叠加 StoplossGuard / MaxDrawdown 软锁：连续止损或回撤超阈则临时锁新仓 |
...
在确定性**硬限**（单票 10% / 行业 30% / 总仓 80% / 单票止损 -8% / 组合 -15% 熔断）之上，再叠加一层从历史事实推导的**软锁**——只临时禁止**新建仓**、从不强制卖出，回测与实盘共用同一套纯逻辑：
